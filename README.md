...
1.2. [Design Statement](#design-statement)

1.3. [Rationale for proposed solution](#rationale-for-proposed-solution)
...
## Design Statement
I will design an innovative web application that will make connecting tutors and students to offer/seek their services easier. The development will be done using the software Flask framework in Python, CSS, and HTML. It will take around 10 weeks to develop and will be evaluated according to the success criteria stated below by the client.

## Rationale for Proposed Solution
In order to revolutionize tutoring services and allow the best-ranked tutors to get the most exposure, instead of the ones with the best advertising, I have decided to develop a web application that serves as an inclusive platform connecting tutors and students and allows the most optimal learning experience.

### Why Web Application?
Web applications are more adequate when developing a platform that offers services like the one I am making because it allows easier access to many different tutors across multiple devices without the need for downloading and installing an application. With a constant addition of new tutors across different subjects, the web application doesn't ask for constant updates. Students that are looking for a tutor can simply access the website through a web browser on their desktop or mobile devices. This increases the potential user number and makes it easier for users to share their experience with tutors with others, allowing high-quality tutors to jump out from the rest. Lastly, web applications can be maintained easily and can be optimized for search engines, making it easier for new users to discover and join this platform both as tutors or as students, depending on their needs.
...
### Why HTML?
HTML is the standard markup language used to create web pages and it provides the structure and content of a website[^7]. HTML is more widely supported than other alternatives such as Flash, making it a better choice for cross-platform compatibility[^8]. Additionally, HTML is more search engine friendly than other technologies such as JavaScript, which can lead to better search engine rankings[^9].
...
2) Tutoring platform will contain useful filter options such as search by name, city, subject, or sort by price.
3) Tutoring platform will allow tutor accounts to upload/delete multiple tutoring posts for different subjects and topics, all on one account.
...
5) Tutoring platform will allow students to see all posts by a specific tutor as well as their reviews.
6) Tutoring service will allow the following specific tutors to see only relevant posts made by them.
7) Will have multiple language features.
# Criteria B: Design
## System Diagram
